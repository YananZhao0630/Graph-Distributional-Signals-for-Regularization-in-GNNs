...
def train(data, model, optimizer, g, eta):
    model.train()
    optimizer.zero_grad()
    pred = model(data)
    loss = F.nll_loss(pred[data.train_mask], data.y[data.train_mask])
    loss = loss + eta * new_loss_function(g,pred)
    loss.backward()
    optimizer.step()
...
def main(args, d_input, d_output, g, eta):
    test_acc_list = []
    for i in range(args.num_expriment):
        ...
        for epoch in range(0, args.epochs):
            train(data, model, optimizer, g, eta)
    ...
    log = 'Model_type: {}, Dateset_name: {}, Experiments: {:03d}, Mean: {:.6f}, std: {:.6f}, eta: {}\n'
    print(log.format(args.model_type, args.dataset, args.num_expriment, np.mean(test_acc_list),
                     np.std(test_acc_list), eta))
...
if __name__ == '__main__':
    ...
    eta = 0.001
    main(args, d_input, d_output, g, eta)
