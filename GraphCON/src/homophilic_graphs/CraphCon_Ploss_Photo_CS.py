...
## new_loss function as regularization
def p_loss(g, x):
...
def train(model, optimizer, data, g, eta):
    model.train()
    optimizer.zero_grad()
    feat = data.x
...
def main(cmd_opt,g, eta):
    #best_opt = good_params_dict[cmd_opt['dataset']]
    ## modify -- the num_splits as 5
    #best_opt['num_splits'] = 5
    opt = {**cmd_opt}
    ...
    for rep in range(opt['num_splits']):
        ...
        for epoch in range(1, opt['epoch']):
            ...
            loss = train(model, optimizer, data, g, eta)
    ...
    if opt['num_splits'] > 1:
        ...
        ## print('final result')
        print('dataset:', opt['dataset'],'test_acc_mean:', test_acc_mean, 'test_acc_std:', test_acc_std, 'eta:', eta)
        if opt['wandb']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_splits', type=int, default=5, help='number of random splits to use')
    # data args
    ...
    opt = vars(args)
    opt['dataset'] = 'CoauthorCS'
    opt['num_splits'] = 5
    opt['function'] = 'laplacian'
    opt['block'] = 'attention'
    opt['method'] = 'symplectic_euler'
    print(opt)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    if opt['dataset'] == 'CoauthorCS':
