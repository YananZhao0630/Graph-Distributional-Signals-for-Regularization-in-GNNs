...
## L_loss function as regularization with 3 hyperparameters
def normalize_adj(A):
    # degree matrix
...
def train(model, optimizer, data, g, eta, m, a, b):
    model.train()
    optimizer.zero_grad()
    feat = data.x
...
def main(cmd_opt,g, eta, m, a, b):
    ...
    for rep in range(opt['num_splits']):
        ...
        for epoch in range(1, opt['epoch']):
            ...
            loss = train(model, optimizer, data, g, eta, m, a, b)
    ...
    if opt['num_splits'] > 1:
        ...
        ## print('final result')
        print('dataset:', opt['dataset'],'test_acc_mean:', test_acc_mean, 'test_acc_std:', test_acc_std, 'eta:', eta, 'm:', m, 'a:', a, 'b:', b)
        if opt['wandb']:
